...
class GmpSeasoningAddition(models.Model):
    _name = "gmp.seasoning.addition"
    _description = "Bỏ gói gia vị (Header)"
    ...
    packaging_machine_id = fields.Char(string="Thiết bị bỏ nêm - bao gói (Mã số)")

    line_ids = fields.One2many(
        comodel_name="gmp.seasoning.addition.line",
...
class GmpSeasoningAdditionLine(models.Model):
    _name = "gmp.seasoning.addition.line"
    _description = "Phân loại (Lines)"

    header_id = fields.Many2one('gmp.seasoning.addition', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.seasoning.addition'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_seasoning_type': self.seasoning_type,
                'default_seasoning_weight': self.seasoning_weight,
                
                'default_deviation': self.deviation,
                'default_quality_of_seasoning': self.quality_of_seasoning,
                'default_condition_of_packaging_equipment': self.condition_of_packaging_equipment,
                'default_printed_info': self.printed_info,
    ...
    def action_open_seasoning_addition_wizard(self):
        ...
        monitoring = self.env['gmp.seasoning.addition'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_seasoning_addition_wizard()

...
class GmpMaterialMixing(models.Model):
    ...
    _description = "Nhào trộn NVL (Header)"
    ...
    # Cặp 1
    select_bon = fields.Boolean(string="Bồn trộn")
    bon_code = fields.Char(string="Mã số bồn")
    # Cặp 2
    select_coi = fields.Boolean(string="Cối trộn")
    coi_code = fields.Char(string="Mã số cối")

    line_ids = fields.One2many(
        comodel_name="gmp.material.mixing.line",
    ...
    def action_open_material_mixing_wizard(self):
        """Hàm mở Wizard cho nút Thêm dòng nhanh (Mobile)"""
        self.ensure_one()
        return {
            'name': 'Thêm dòng nhanh (Mobile)',
            'type': 'ir.actions.act_window',
            'res_model': 'gmp.material.mixing.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
...
class GmpMaterialMixingLine(models.Model):
    _name = "gmp.material.mixing.line"
    _description = "Cấp liệu chế biến (Lines)"

    header_id = fields.Many2one('gmp.material.mixing', ondelete="cascade")
    ...
    mixing_speed = fields.Float(string="Tốc độ nhào trộn (vòng/phút)")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.material.mixing'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_mixing_time': self.mixing_time,
                'default_mixing_speed': self.mixing_speed,
                'default_dough_temperature': self.dough_temperature,
                'default_dough_status': self.dough_status,
    ...
    def action_open_material_mixing_wizard(self):
        ...
        monitoring = self.env['gmp.material.mixing'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_material_mixing_wizard()

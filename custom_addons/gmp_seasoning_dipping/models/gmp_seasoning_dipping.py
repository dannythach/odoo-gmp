...
class GmpSeasoningDippingLine(models.Model):
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.seasoning.dipping'].browse(
    ...
    def action_open_seasoning_dipping_wizard(self):
        ...
        monitoring = self.env['gmp.seasoning.dipping'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_seasoning_dipping_wizard()

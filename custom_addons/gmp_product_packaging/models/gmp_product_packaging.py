...
class GmpProductPackaging(models.Model):
    ...
    _description = "Bao gói sản phẩm (Header)"
    ...
    line_ids = fields.One2many(
        comodel_name="gmp.product.packaging.line",
...
class GmpProductPackagingLine(models.Model):
    _name = "gmp.product.packaging.line"
    _description = "Phân loại (Lines)"

    header_id = fields.Many2one('gmp.product.packaging', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.product.packaging'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_packaging_type': self.packaging_type,
                'default_cutter_speed': self.cutter_speed,
                'default_temperature_at_ends': self.temperature_at_ends,
                'default_temperature_for_middle': self.temperature_for_middle,
                'default_total_products': self.total_products,
                'default_inspected_products': self.inspected_products,
                'default_passed_products': self.passed_products,
                'default_defective_products': self.defective_products,
                'default_defect_types': self.defect_types,
    ...
    def action_open_product_packaging_wizard(self):
        ...
        monitoring = self.env['gmp.product.packaging'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_product_packaging_wizard()

...
class GmpMetalDetection(models.Model):
    _name = "gmp.metal.detection"
    _description = "Dò kim loại sản phẩm (Header)"
    ...
    line_ids = fields.One2many(
        comodel_name="gmp.metal.detection.line",
...
class GmpMetalDetectionLine(models.Model):
    _name = "gmp.metal.detection.line"
    _description = "Phân loại (Lines)"

    header_id = fields.Many2one('gmp.metal.detection', ondelete="cascade")
    ...
    quantity_of_products_scanned = fields.Float(string="SL SP dò")
    quantity_of_products_detected = fields.Float(string="SL SP phát hiện")
    fe_detected = fields.Float(string="SL SP phát hiện Fe")
    nonfe_detected = fields.Float(string="SL SP phát hiện Non-Fe")
    sus_detected = fields.Float(string="SL SP phát hiện Sus")

    re_scanned = fields.Float(string="SL dò lại")
    isolated = fields.Float(string="SL cô lập & xử lý")
    opened = fields.Float(string="SL mở gói & xử lý")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.metal.detection'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_machine_type': self.machine_type.id,
                'default_fe_type': self.fe_type,
                'default_nonfe_type': self.nonfe_type,
                'default_sus_type': self.sus_type,
                'default_quantity_of_products_scanned': self.quantity_of_products_scanned,
                'default_quantity_of_products_detected': self.quantity_of_products_detected,
                'default_fe_detected': self.fe_detected,
                'default_nonfe_detected': self.nonfe_detected,
                'default_sus_detected': self.sus_detected,
                
                'default_re_scanned': self.re_scanned,
                'default_isolated': self.isolated,
                'default_opened': self.opened,
    ...
    def action_open_metal_detection_wizard(self):
        ...
        monitoring = self.env['gmp.metal.detection'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_metal_detection_wizard()

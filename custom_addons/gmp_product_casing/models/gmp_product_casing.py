...
class GmpProductCasing(models.Model):
    ...
    _description = "Đóng thùng sản phẩm (Header)"
    ...
    machine_id = fields.Char(string="Thiết bị đóng thùng (Mã số)")

    line_ids = fields.One2many(
        comodel_name="gmp.product.casing.line",
        inverse_name="header_id",
        string="Chi tiết các dòng"
    )
...
class GmpProductCasingLine(models.Model):
    _name = "gmp.product.casing.line"
    _description = "Phân loại (Lines)"

    header_id = fields.Many2one('gmp.product.casing', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.product.casing'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_specifications': self.specifications,
                'default_quantity_case': self.quantity_case,
                'default_inspected_cases': self.inspected_cases,
                'default_passed_cases': self.passed_cases,
                'default_rejected_cases': self.rejected_cases,
                'default_defect_type': self.defect_type,
    ...
    def action_open_metal_detection_wizard(self):
        ...
        monitoring = self.env['gmp.product.casing'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_product_casing_wizard()

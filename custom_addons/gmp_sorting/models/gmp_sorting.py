...
class GmpSorting(models.Model):
    _name = "gmp.sorting"
    _description = "Phân loại (Header)"
...
class GmpSortingLine(models.Model):
    _name = "gmp.sorting.line"
    _description = "Phân loại (Lines)"

    header_id = fields.Many2one('gmp.sorting', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.cooling'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_number_of_noodle_cakes': self.number_of_noodle_cakes,
                'default_number_of_inspected_cakes': self.number_of_inspected_cakes,
                'default_number_of_qualified_cakes': self.number_of_qualified_cakes,
                'default_quantity_of_downgraded_cakes': self.quantity_of_downgraded_cakes,
                'default_substandard_defect_name': self.substandard_defect_name,
                'default_reject_defect_name': self.reject_defect_name,
                'default_reject_rate': self.reject_rate,
    ...
    def action_open_sorting_wizard(self):
        ...
        monitoring = self.env['gmp.sorting'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_sorting_wizard()

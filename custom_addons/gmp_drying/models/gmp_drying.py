...
class GmpDrying(models.Model):
    ...
    air_supply_system_id = fields.Char(string="Hệ thống cấp khí (Mẫ số)")
    fan_id = fields.Char(string="Các thiết bị quạt (Mã số)")

    line_ids = fields.One2many(
        comodel_name="gmp.drying.line",
...
class GmpDryingLine(models.Model):
    _name = "gmp.drying.line"
    _description = "Quạt ráo (Lines)"

    header_id = fields.Many2one('gmp.drying', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.drying'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_drying_time': self.drying_time,
                'default_blowing_speed': self.blowing_speed,
                'default_air_tempature': self.air_tempature,
                'default_airflow_rate': self.airflow_rate,
                'default_equipment_status': self.equipment_status,
    ...
    def action_open_drying_wizard(self):
        ...
        monitoring = self.env['gmp.drying'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_drying_wizard()

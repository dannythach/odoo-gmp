...
class GmpCooling(models.Model):
    _name = "gmp.cooling"
    _description = "Thổi nguội (Header)"
    ...
    cooling_chamber_id = fields.Char(string="Buồng thổi nguội (Mã số)")
    fan_id = fields.Char(string="Các Thiết bị quạt ly tâm (Mã số)")

    line_ids = fields.One2many(
        comodel_name="gmp.cooling.line",
...
class GmpCoolingLine(models.Model):
    _name = "gmp.cooling.line"
    _description = "Thổi nguội (Lines)"

    header_id = fields.Many2one('gmp.cooling', ondelete="cascade")
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_chamber_temperature': self.chamber_temperature,
                'default_cooling_time': self.cooling_time,
                'default_vertical_blowing': self.vertical_blowing,
                'default_horizontal_blowing': self.horizontal_blowing,
                'default_cooling_temperature': self.cooling_temperature,
    ...
    def action_open_cooling_wizard(self):
        ...
        monitoring = self.env['gmp.cooling'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_cooling_wizard()

...
class GmpCuttingLine(models.Model):
    _name = "gmp.cutting.line"
    _description = "Cắt định lượng (Lines)"

    header_id = fields.Many2one('gmp.cutting', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.cutting'].browse(
    ...
    def action_open_cutting_wizard(self):
        ...
        monitoring = self.env['gmp.cutting'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_cutting_wizard()

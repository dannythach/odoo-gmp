...
class GmpMolding(models.Model):
    _name = "gmp.molding"
    _description = "Vô khuôn (Header)"
    ...
    molding_system_id = fields.Char(string="Hệ thống dây khuôn (Mã số)")
    mold_type = fields.Char(string="Loại khuôn (QC/Kich thước)")

    line_ids = fields.One2many(
        comodel_name="gmp.molding.line",
...
class GmpMoldingLine(models.Model):
    _name = "gmp.molding.line"
    _description = "Nhúng nước lèo (Lines)"

    header_id = fields.Many2one('gmp.molding', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.molding'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_noodle_cake_size': self.noodle_cake_size,
                'default_size_tolerance': self.size_tolerance,
                'default_weight_per_cake': self.weight_per_cake,
                'default_weight_tolerance': self.weight_tolerance,
                'default_molding_status': self.molding_status,  
    ...
    def action_open_molding_wizard(self):
        ...
        monitoring = self.env['gmp.molding'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_molding_wizard()

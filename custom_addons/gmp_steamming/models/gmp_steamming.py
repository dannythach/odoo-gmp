...
class GmpSteamming(models.Model):
    ...
    chamber_id = fields.Char(string="Buồng/Bồn hấp (Mẫ số)")
    pipe_id = fields.Char(string="Các ống dẫn hơi (Mã số)")

    line_ids = fields.One2many(
        comodel_name="gmp.steamming.line",
...
class GmpSteammingLine(models.Model):
    _name = "gmp.steamming.line"
    _description = "Hấp (Lines)"

    header_id = fields.Many2one('gmp.steamming', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.steamming'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_steamming_time': self.steamming_time,
                'default_vapor_pressurer': self.vapor_pressurer,
                'default_chamber_pressuer': self.chamber_pressuer,
                'default_gelatinization': self.gelatinization,
                'default_moisture': self.moisture,
                'default_wip_status': self.wip_status,
    ...
    def action_open_steamming_wizard(self):
        ...
        monitoring = self.env['gmp.steamming'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_steamming_wizard()

...
class GmpFrying(models.Model):
    _name = "gmp.frying"
    _description = "Chiên (Header)"
    ...
    frying_chain_system_id = fields.Char(string="Hệ thống dây chuyền chiên (Mã số)")
    frying_chain_info = fields.Char(string="Dây khuôn (QC/ Kích thước)")
    fryer = fields.Char(string="Chảo chiên (Mã số)")

    line_ids = fields.One2many(
        comodel_name="gmp.frying.line",
...
class GmpFryingLine(models.Model):
    _name = "gmp.frying.line"
    _description = "Chiên (Lines)"

    header_id = fields.Many2one('gmp.frying', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.molding.adjustment'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_tempareture': self.tempareture,
                'default_frying_time': self.frying_time,
                'default_noodle_moisture': self.noodle_moisture,
                'default_oil_indices': self.oil_indices,
                'default_shortening_color': self.shortening_color,
                
                'default_bht_level': self.bht_level,
                'default_lipid_level': self.lipid_level,
                
                'default_sfp_status': self.sfp_status,
                'default_machine_condition': self.machine_condition,
    ...
    def action_open_frying_wizard(self):
        ...
        monitoring = self.env['gmp.frying'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_frying_wizard()

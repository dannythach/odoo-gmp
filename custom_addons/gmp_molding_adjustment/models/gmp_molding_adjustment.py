...
class GmpMoldingAdjustment(models.Model):
    ...
    adjustment_stick = fields.Char(string="Đũa sửa khuôn (QC/Kich thước)")

    line_ids = fields.One2many(
        comodel_name="gmp.molding.adjustment.line",
...
class GmpMoldingAdjustmentLine(models.Model):
    _name = "gmp.molding.adjustment.line"
    _description = "Sửa khuôn (Lines)"

    header_id = fields.Many2one('gmp.molding.adjustment', ondelete="cascade")
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_adjustment_stick_diameter': self.adjustment_stick_diameter,
                'default_adjustment_stick_length': self.adjustment_stick_length,
                'default_distance_stick_mold': self.distance_stick_mold,
                'default_air_flow_rate': self.air_flow_rate,
                'default_air_pressure': self.air_pressure,
                'default_fan_speed': self.fan_speed,
                'default_fan_wind_speed': self.fan_wind_speed,
    ...
    def action_open_molding_adjustment_wizard(self):
        ...
        monitoring = self.env['gmp.molding.adjustment'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_molding_adjustment_wizard()

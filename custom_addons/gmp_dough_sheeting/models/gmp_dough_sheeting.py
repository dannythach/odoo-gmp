...
class GmpDoughSheeting(models.Model):
    ...
    dough_mass = fields.Char(string="Khối bột")
    weight = fields.Float(string="Trọng lượng (kg)")
    machine_id = fields.Char(string="Máy cán (Mã số)")
    roller_id = fields.Char(string="Trục cán (Mã số)")
    conveyor_belt = fields.Char(string="Băng tải")

    line_ids = fields.One2many(
        comodel_name="gmp.dough.sheeting.line",
...
class GmpDoughSheetingLine(models.Model):
    _name = "gmp.dough.sheeting.line"
    _description = "Cán bột (Lines)"

    header_id = fields.Many2one('gmp.dough.sheeting', ondelete="cascade")
    ...
    def _onchange_material_id_load_data(self):
        for record in self:
            ...
            # 2. Lấy header
            header = record.header_id or self.env['gmp.dough.sheeting'].browse(
    ...
    def action_edit_line(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_rough_roller_id': self.rough_roller_id,
                'default_rough_speed': self.rough_speed,
                'default_rough_thickness': self.rough_thickness,

                'default_final_roller_id': self.final_roller_id,
                'default_final_speed': self.final_speed,
                'default_final_thickness': self.final_thickness,
                'default_final_status': self.final_status,
    ...
    def action_open_dough_sheeting_wizard(self):
        ...
        monitoring = self.env['gmp.dough.sheeting'].browse(self._context.get('active_id'))
        if not monitoring:
            # Trường hợp bản ghi mới chưa lưu, lấy qua field Many2one
            monitoring = self.header_id
            
        return monitoring.action_open_dough_sheeting_wizard()
